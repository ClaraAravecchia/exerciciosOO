...
CREATE  VIEW `gerente` AS select `funcionario`.`ID_FUNCIONARIO` AS `id_gerente`,`funcionario`.`NOME` AS `nome`,`funcionario`.`SOBRENOME` AS `sobrenome`,`funcionario`.`EMAIL` AS `email`,`funcionario`.`TELEFONE` AS `telefone`,`funcionario`.`DATA_CONTRATACAO` AS `data_contratacao`,`funcionario`.`ID_FUNCAO` AS `id_funcao`,`funcionario`.`SALARIO` AS `salario`,`funcionario`.`COMISSAO` AS `comissao`,`funcionario`.`ID_DEPARTAMENTO` AS `id_departamento` from `funcionario`;
